...
def minNext(start, unmarked):
	...
	for x in range(0, endPoint, interval):
		...
		if (minDist == 0):
			...
			secMinVal = list(minVal)
			minDist = selectDist
			minVal = list(selectVal)
			if (secMinVal != None):
				secMinDist = calcDistance(minVal, secMinVal)
				if (secMinDist > 25):
					secMinDist = 0
					secMinVal = None
			#secMinDist = minDist
	...
	return minDist, minVal, secMinDist, secMinVal
...
def calcMin(start, graph):
	...
	while (len(unmarked) > 0):
		nextD, nextV, secD, secV = minNext(currentV, unmarked)

		distance = distance + nextD + secD
